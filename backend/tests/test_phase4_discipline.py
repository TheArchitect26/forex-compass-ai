...
class Phase4DisciplineTests(unittest.TestCase):
    def test_invalidation_hit_before_tp_sl(self):
        res = evaluate_outcome("BUY", "EUR/USD", 1.1000, 1.0950, 1.1100, 1.0980, [{"high": 1.1010, "low": 1.0970}], datetime.utcnow()+timedelta(hours=1), datetime.utcnow())
    ...
    def test_duplicate_cooldown(self):
        self.assertTrue(is_duplicate_recent(datetime.utcnow()-timedelta(minutes=5), 30))
